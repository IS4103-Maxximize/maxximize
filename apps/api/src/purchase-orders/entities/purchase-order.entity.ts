...
export class PurchaseOrder {
  ...
  @ManyToOne(
    () => Organisation,
    (currentOrganisation) => currentOrganisation.purchaseOrders
  )
  ...
  @ManyToOne(
    () => Organisation,
    (supplier) => supplier.receivedPurchaseOrders,
    {
      nullable: true,
    }
  )
  ...
  @OneToMany(
    ...
    (poLineItem) => poLineItem.purchaseOrder,
    { cascade: true }
  )
  ...
  @OneToMany(
    () => FollowUpLineItem,
    (followUpLineItem) => followUpLineItem.purchaseOrder,
    { cascade: true }
  )
  ...
  @OneToMany(
    () => ProductionRequest,
    (prodRequest) => prodRequest.purchaseOrder,
    { nullable: true }
  )
  prodRequests: ProductionRequest[];
  @OneToMany(
    () => DeliveryRequest,
    (deliveryRequest) => deliveryRequest.purchaseOrder
  )
  deliveryRequests: DeliveryRequest[];

  @OneToMany(
    () => ReservationLineItem,
    (reservationLineItem) => reservationLineItem.purchaseOrder,
    {
      cascade: true,
    }
  )
  ...
  @OneToOne(() => Invoice, (invoice) => invoice.po, { nullable: true })
  invoice: Invoice;
}
