...
export class MembershipsService {
  ...
  async create(createMembershipDto: CreateMembershipDto) {
    ...
    const organisationToBeAdded = await this.organisationService.findOne(
      organisationId
    );
  ...
  async findMembershipByCustomer(customerId: string) {
    try {
      ...
      throw new NotFoundException(
        `customer with id: ${customerId} cannot be found!`
      );
    }
  }
  ...
  async getCustomer(customerId: string) {
    const retrievedCustomer = (await this.stripe.customers.retrieve(
      customerId
    )) as Stripe.Customer;
  ...
  async getProduct(productId: string) {
    ...
    if (product.length === 1) {
      ...
      throw new NotFoundException(
        `product with id: ${productId} cannot be found!`
      );
    }
  }
  ...
  async handleEvents(body: any) {
    ...
    if (type === 'customer.subscription.created') {
      ...
      const organisation = await this.organisationService.findOrgByEmail(
        customer.email
      );
      ...
      const currentSubscriptionPlan = await this.getSubscriptionPlan(
        stripeProduct
      );
      ...
      let membership = await this.findMembershipByCustomer(customerId);
      const newStatus = pause_collection
      ...
      const membership = await this.findMembershipByCustomer(customerId);
      await this.remove(membership.id);
  ...
  async getInvoicesOfCustomer(customerId: string) {
    ...
    const parsedInvoices = allInvoices.map((invoice) => {
      ...
      return {
        ...
        status,
        billingReason:
          (billing_reason as string) === 'subscription_create'
            ? 'Subscription'
            : 'Others',
  ...
  async getCustomerPaymentMethods(customerId: string) {
    const paymentMethods = await this.stripe.customers.listPaymentMethods(
      customerId,
      { type: 'card' } //set as static type first, might change to dynamic if needed
    );
  ...
  async getCustomerSubscriptions(customerId: string) {
    ...
    const parsedSubscriptionsPromises = subscriptions.map(
      async (subscription) => {
