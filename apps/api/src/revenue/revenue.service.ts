...
export class RevenueService implements OnModuleInit {
  ...
  async getRevenueByDate(getRevenueDto: GetRevenueDto) {
    ...
    const allInvoices = await this.invoicesService.findSentInvoicesByOrg(
      organisationId
    );
    ...
    if (type === 'month') {
      ...
      if (!range) {
        filteredInvoices = closedInvoices.filter((invoice) => {
          ...
          return (
            dateOfPaymentReceived.getMonth() === month &&
  ...
  async paymentToStripe(revenueCommisionDto: RevenueCommisionDto) {
    const { customerId, amount, paymentMethod, currency, description } =
      revenueCommisionDto;
    ...
    if (revenueArray.length > 0) {
      ...
      if (bracket) {
        ...
        const paymentAmount = Math.round(
          (percentage / 100) * grossRevenueAmount
        );
      ...
      if (revenueArray.length > 0) {
        ...
        if (bracket) {
          ...
          const revenueCommisionDto = {
            ...
            currency: 'sgd',
            description: `this is the commision paid for month: ${
    ...
    if (revenueArray.length === 0) {
      ...
      for (const lineItem of lineItems) {
        ...
        if (!map.has(supplierKey)) {
          ...
          const newContribution = (currentValue.contribution +=
            lineItem.amount);
          const newValue = {
            ...currentValue,
  ...
  async getSalesAmount(getSalesAmountDto: GetSalesAmountDto) {
    ...
    const earliestDate = new Date(
      cloneDate.setDate(cloneDate.getDate() - (days - 1))
    );
    ...
    const allSentInvoices = await this.invoicesService.findSentInvoicesByOrg(
      organisationId
    );
    const currentMonthInvoices = allSentInvoices.filter((invoice) => {
      ...
      return (
        paymentMonth >= earliestMonth &&
        paymentMonth <= latestMonth &&
        paymentYear >= earliestYear &&
        paymentYear <= latestYear
      );
    });
    const previousYearButSameMonthInvoices = allSentInvoices.filter(
      (invoice) => {
        const invoiceDate = invoice.paymentReceived;
        const paymentMonth = invoiceDate.getMonth();
        const paymentYear = invoiceDate.getFullYear();
    ...
    for (let i = 0; i < days; i++) {
      ...
      const currentYearInvoicesThatMatchDate = currentMonthInvoices.filter(
        (invoice) =>
      ...
      const previousYearInvoicesThatMatchDate =
        previousYearButSameMonthInvoices.filter(
          (invoice) =>
            invoice.paymentReceived.getDate() === day &&
            invoice.paymentReceived.getMonth() === month &&
