...
export class BulkDiscountsService {
  ...
  async create(createBulkDiscountDto: CreateBulkDiscountDto) {
    const {bulkType, bulkDiscountRangeDtos, organisationId, scheduleActivation} = createBulkDiscountDto
    ...
    const newBulkDiscount = this.bulkDiscountsRepository.create({
      ...
      created: new Date(),
      scheduleActivation: scheduleActivation ? new Date(scheduleActivation) : null
    })
    const savedBulkDiscount = await this.bulkDiscountsRepository.save(newBulkDiscount)
    ...
    if (scheduleActivation) {
      ...
      const job = new CronJob(dateToActivate, async() => {
        await this.update(savedBulkDiscount.id, {isActive: true, scheduleActivation: null})
      })
      this.schedulerRegistry.addCronJob(`${savedBulkDiscount.id}-activateBulkDiscount`, job);
      job.start()
      //add chron job to database
      await this.chronJobService.create({scheduledDate: dateToActivate, type: ChronType.BULKDISCOUNT, targetId: savedBulkDiscount.id})
    }
  ...
  async update(id: number, updateBulkDiscountDto: UpdateBulkDiscountDto) {
    ...
    for (const [key, value] of keyValues) {
      if (key === 'isActive' && value) {
        ...
        const allBulkDiscounts = await this.findAllByOrg(bulkDiscount.organisationId)
        const activeBulkDiscount = allBulkDiscounts.find(discount => discount.isActive)
        if (activeBulkDiscount) {
