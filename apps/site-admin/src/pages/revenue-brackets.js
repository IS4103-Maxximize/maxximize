...
export const RevenueBrackets = (props) => {
  ...
  const handleDelete = async () => {
    ...
    const deleteRevenueBracket = fetch(deleteUrl, { method: 'DELETE' }).then(res => res.json());
    const updateRevenueBracket = fetch(updateUrl, updateRequestOptions).then(res => res.json());
  ...
  const handleUpdate = async (only) => {
    ...
    await Promise.all(promises)
      ...
      .catch(err => handleAlertOpen('Failed to update Revenue Bracket', 'error'));
  }
  ...
  const handleCreate = async () => {
    ...
    const createRevenueBracket = fetch(baseUrl, createRequestOptions).then(res => res.json());
    const updateRevenueBracket = fetch(updateUrl, updateRequestOptions).then(res => res.json());
    ...
    await Promise.all([createRevenueBracket, updateRevenueBracket])
      ...
      .catch(err => handleAlertOpen('Failed to create Revenue Bracket', 'error'));
  }
          ...
          >
              ...
              <CardContent>
                <Grid container spacing={2}>
                  <Grid item md={6} xs={12}>
                    <TextField
                      fullWidth
                      error={Boolean(formik.touched.newStart && formik.errors.newStart)}
                      helperText={formik.touched.newStart && formik.errors.newStart}
                  ...
                  <Grid item md={6} xs={12}>
                    <TextField
                      fullWidth
                      error={Boolean(formik.touched.newCommisionRate && formik.errors.newCommisionRate)}
                      helperText={formik.touched.newCommisionRate && formik.errors.newCommisionRate}
            ...
            {brackets.length > 0 ? (
              brackets.map((bracket, index) => (
                  ...
                  <CardContent>
                    <Grid container spacing={2} alignItems="center">
                      ...
                      <Grid item md={4} xs={12}>
                        <TextField
                          fullWidth
                          error={Boolean(formik.touched.commisionRate && formik.errors.commisionRate)}
                          helperText={formik.touched.commisionRate && formik.errors.commisionRate}
                          label="Commission Rate (%)"
                          margin="normal"
                          name="commisionRate"
                          type="number"
                ...
                {index < brackets.length-1 && <KeyboardDoubleArrowDownIcon />}
                </>
              ))
            ) : (
              <Card
